...
bash .system/auto_correc_program.sh $FILE $ASSIGN "L'eSPrit nE peUt plUs pRogResSer s'Il staGne et sI peRsIsTent VAnIte et auto-justification."
if [ -e .system/grading/traceback ];then
    mv .system/grading/traceback .
	exit 1
fi

bash .system/auto_correc_program.sh $FILE $ASSIGN "S'enTOuRer dE sECreT eSt uN sIGnE De mAnQuE De coNNaiSSanCe.  "
if [ -e .system/grading/traceback ];then
    mv .system/grading/traceback .
	exit 1
fi

bash .system/auto_correc_program.sh $FILE $ASSIGN "3:21 Ba  tOut  moUn ki Ka di KE m'en Ka fe fot"
if [ -e .system/grading/traceback ];then
    mv .system/grading/traceback .
	exit 1
fi
